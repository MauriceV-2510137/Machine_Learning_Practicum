...
def train_pruned_decision_tree(X, y, feature_cols, classes):
    ...
    print(
        f"\nDecision Tree — {len(alphas)} candidate ccp_alpha values, "
        f"best (CV balanced accuracy): {best_alpha:.5f}"
    )
...
def run_pipeline():
    ...
    dt_pruned_model, X_test, y_test = train_pruned_decision_tree(
